...
#if defined(NDEBUG)
# error "DCcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3f6e2e0d63a910815ea9c01dca6080fce86416538b9de4e2a4b0f77af2fc0cc4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DCcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DCcoin Signed Message:\n";
...
{
    // DCcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 2000000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 2100000); // DCcoin: 2M blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // DCcoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // DCcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DCcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DCcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xef46ba764de201f0c61caae2d8342a711b97e8f1891ba3ffaff89985d1b30eb4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Anger always comes from a lack of conversation.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0413f64d991f1e522572e79b4b0caf667e7988981fd3f0e3022354357e2830555e06c6f27caf2c29212dd6582ca2c27813b6b20c7d47cec569046c44d170aae8c6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520421766;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084795847;

        if (fTestNet)
        {
            block.nTime    = 1520421737;
            block.nNonce   = 386149285;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc52810fd750980b19c336331d4c0bd8b1eaddecf58261d3eabd02b50a7767a67"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // DCcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DCcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DCcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DCcoinMiner(CWallet *pwallet)
{
    printf("DCcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dccoin-miner");
    ...
    try { loop {
        ...
        printf("Running DCcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DCcoinMiner, pwallet));
}
